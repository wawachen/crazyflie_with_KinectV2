...
class livePlotter():
    def __init__(self):
        ...
        rospy.Subscriber('/crazyflie/bcf_state', KalmanPositionEst, self.state_callback)
        rospy.Subscriber('bcf_action', ActionPub, self.action_callback)
        rospy.Subscriber('/crazyflie/z_range', zRangeData, self.zrange_callback)
    ...
    def zrange_callback(self, data):
        self.concentration = data.down
        # print("intensity = {}".format(self.intensity))
        self.plotArray = np.vstack(([self.plotArray, np.array([self.x, self.y, self.concentration])]))
